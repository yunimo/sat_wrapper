...
(*** ルール ***)
(* n×nの正方格子で先手と後手が順に格子のマスを1つずつ選択する *)
(* マスの選択をn×n回行った時に，縦か横のある1列のマスをすべて先手が選んでいれば，先手の勝利となる *)
(* それを阻止できれば，後手の勝利となる *)

(*** n目並べにおける後手必勝アルゴリズム：先手が選んだ番号を後手も選ぶことで後手が必ず勝利する ***)
...
(* Trueが割り当てられた変数のリストをソートしたリストを返す  *)
let sort_tuples lst =
...
let l2 = ands_ (forInc 1 n (fun i -> forInc 1 n (fun j -> )
